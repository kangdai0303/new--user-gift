...
class NewUserGiftPage {
    ...
    getAndroidDownloadUrl(userAgent) {
        ...
        if (/huawei/i.test(userAgent) || /honor/i.test(userAgent) || /hms/i.test(userAgent)) {
            return {
                intent: `intent://details?id=${packageName}#Intent;scheme=appmarket;package=com.huawei.appmarket;end`,
        ...
        if (/xiaomi/i.test(userAgent) || /mi\s/i.test(userAgent) || /redmi/i.test(userAgent)) {
            return {
                intent: `intent://details?id=${packageName}#Intent;scheme=market;package=com.xiaomi.market;end`,
        ...
        if (/oppo/i.test(userAgent) || /oneplus/i.test(userAgent)) {
            return {
                intent: `intent://details?id=${packageName}#Intent;scheme=market;package=com.oppo.market;end`,
        ...
        if (/vivo/i.test(userAgent) || /iqoo/i.test(userAgent)) {
            return {
                intent: `intent://details?id=${packageName}#Intent;scheme=market;package=com.bbk.appstore;end`,
        ...
        // 其他Android手机默认跳转应用宝
        return {
            intent: `intent://details?id=${packageName}#Intent;scheme=market;package=com.tencent.android.qqdownloader;end`,
    ...
    directDownload(downloadInfo) {
        ...
        if (typeof downloadInfo === 'string') {
            try {
                ...
                this.showNotification('正在跳转到App Store...', 'success');
            } catch (error) {
        ...
        // Android设备 - 多重尝试拉起应用商店APP
        const { intent, fallback, marketName } = downloadInfo;
        const packageName = 'uni.UNI6384A90';
        ...
        const handleVisibilityChange = () => {
            if (document.visibilityState === 'hidden' && !hasJumped) {
                hasJumped = true;
                this.showNotification(`${marketName}已拉起成功！`, 'success');
        ...
        // 监听页面焦点变化
        const handleBlur = () => {
            if (!hasJumped && Date.now() - startTime < 2000) {
                hasJumped = true;
                this.showNotification(`${marketName}已拉起成功！`, 'success');
        ...
        const tryMethods = [
            ...
            () => {
                attemptCount++;
                console.log(`尝试方法${attemptCount}: Intent URL`);
                window.location.href = intent;
            },
            // 方法2: 通用market协议
            () => {
                attemptCount++;
                console.log(`尝试方法${attemptCount}: 通用market协议`);
                window.location.href = `market://details?id=${packageName}`;
            },
            // 方法3: 创建隐藏iframe尝试
            () => {
                attemptCount++;
                console.log(`尝试方法${attemptCount}: iframe方式`);
        ...
        const tryNextMethod = () => {
            if (methodIndex < tryMethods.length && !hasJumped) {
                try {
                    tryMethods[methodIndex]();
                    methodIndex++;
                    
                    // 800ms后尝试下一种方法
                    setTimeout(tryNextMethod, 800);
                } catch (error) {
                    console.log(`方法${methodIndex + 1}失败:`, error);
                    methodIndex++;
                    setTimeout(tryNextMethod, 100);
                }
            } else if (!hasJumped && methodIndex >= tryMethods.length) {
                // 所有方法都尝试过了，使用fallback
                console.log('所有拉起方法都失败，使用fallback');
                this.showNotification(`正在打开${marketName}网页版...`, 'warning');
        ...
        setTimeout(() => {
            ...
            if (!hasJumped && document.visibilityState === 'visible') {
                // 如果页面仍然可见且没有成功跳转，显示fallback
                if (methodIndex >= tryMethods.length) {
                    this.showNotification('应用商店拉起可能失败，已打开网页版', 'info');
                }
            }
        }, 3000);
    }
