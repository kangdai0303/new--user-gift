...
class NewUserGiftPage {
    ...
    getAndroidDownloadUrl(userAgent) {
        ...
        if (/huawei/i.test(userAgent) || /honor/i.test(userAgent) || /hms/i.test(userAgent)) {
            return 'https://appgallery.huawei.com/app/C112125043';
        }
        
        // 检测小米手机
        if (/xiaomi/i.test(userAgent) || /mi\s/i.test(userAgent) || /redmi/i.test(userAgent)) {
            return 'https://app.mi.com/details?id=uni.UNI6384A90';
        }
        
        // 检测OPPO手机
        if (/oppo/i.test(userAgent) || /oneplus/i.test(userAgent)) {
            return 'https://app.cdo.oppomobile.com/home/detail?app_id=32592129';
        }
        
        // 检测Vivo手机
        if (/vivo/i.test(userAgent) || /iqoo/i.test(userAgent)) {
            return 'https://h5coml.vivo.com.cn/h5coml/appdetail_h5/browser_v2/index.html?appId=3805866';
        }
        
        // 其他Android手机默认跳转应用宝
        return 'https://a.app.qq.com/o/simple.jsp?pkgname=uni.UNI6384A90';
    }
    ...
    directDownload(url) {
        try {
            ...
            this.showNotification('正在跳转到应用商店...', 'success');
        } catch (error) {
