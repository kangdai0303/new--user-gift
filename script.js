...
class NewUserGiftPage {
    ...
    handleDownloadApp() {
        ...
        // 获取推广员的专属下载链接
        const downloadUrl = this.getPromoterDownloadLink();
    ...
    // 获取推广员专属下载链接
    getPromoterDownloadLink() {
        // 从URL参数或推广员信息中获取下载链接
        const promoterInfo = window.tracker ? window.tracker.promoterInfo : this.getPromoterFromURL();
        
        // 获取推广员ID
        const promoterId = promoterInfo.promoterId || 'direct';
        
        // 从配置文件中获取对应的下载链接
        if (window.PROMOTERS_CONFIG && window.PROMOTERS_CONFIG[promoterId]) {
            return window.PROMOTERS_CONFIG[promoterId].downloadLink;
        }
        
        // 如果没有找到，使用默认配置
        return window.DEFAULT_CONFIG ? window.DEFAULT_CONFIG.downloadLink : 'https://yikao.gaotuyk.com/#/pages/recruit/notice_detail?id=489&share=app&share=h5&invitationCode=p_default';
    }
    ...
    getPromoterInfo() {
        ...
        if (window.PROMOTERS_CONFIG && window.PROMOTERS_CONFIG[promoterId]) {
            return {
                ...promoterInfo,
                ...window.PROMOTERS_CONFIG[promoterId]
            };
        }
    ...
    getPromoterFromURL() {
        ...
        return {
            promoterId: urlParams.get('pid') || urlParams.get('promoter_id') || 'direct',
            promoterName: urlParams.get('promoter_name') || urlParams.get('name') || '直接访问',
    ...
    // 直接下载跳转 - 简化版本，直接跳转到推广链接
    directDownload(downloadUrl) {
        try {
            this.showNotification('正在跳转到下载页面...', 'success');
            
            // 直接跳转到推广员的专属链接
            window.open(downloadUrl, '_blank', 'noopener,noreferrer');
            
            // 记录跳转成功
            console.log('跳转到推广链接:', downloadUrl);
            
        } catch (error) {
            console.log('跳转失败:', error);
            this.showNotification('跳转失败，请稍后再试', 'error');
            
            // 备用方案：直接修改当前页面地址
            try {
                window.location.href = downloadUrl;
            } catch (fallbackError) {
                console.log('备用跳转也失败:', fallbackError);
            }
        }
    }
...
class PromotionTracker {
    ...
    async trackDownload() {
        const downloadLink = this.getPromoterDownloadLink();
        await this.sendTracking({
    ...
    getPromoterDownloadLink() {
        // 从配置文件中获取下载链接（与主类保持一致）
        if (window.PROMOTERS_CONFIG && window.PROMOTERS_CONFIG[this.promoterInfo.promoterId]) {
            return window.PROMOTERS_CONFIG[this.promoterInfo.promoterId].downloadLink;
        }
    ...
    async sendTracking(additionalData) {
        ...
        try {
            ...
            localStorage.setItem('tracking_history', JSON.stringify(trackingHistory.slice(-100))); // 保存更多记录
            
            // 按推广员分组统计
            const promoterStats = JSON.parse(localStorage.getItem('promoter_stats') || '{}');
            if (!promoterStats[this.promoterInfo.promoterId]) {
                promoterStats[this.promoterInfo.promoterId] = {
                    promoterName: this.promoterInfo.promoterName,
                    totalViews: 0,
                    totalClicks: 0,
                    lastActive: null
            ...
            if (additionalData.action === 'page_view') {
                promoterStats[this.promoterInfo.promoterId].totalViews++;
            } else if (additionalData.action === 'download_click') {
                promoterStats[this.promoterInfo.promoterId].totalClicks++;
            }
            
            promoterStats[this.promoterInfo.promoterId].lastActive = new Date().toISOString();
            localStorage.setItem('promoter_stats', JSON.stringify(promoterStats));
            
            console.log('📊 追踪数据:', data);
            console.log('📈 推广员统计:', promoterStats[this.promoterInfo.promoterId]);
    ...
    // 获取推广统计报告
    getPromotionReport() {
        const stats = JSON.parse(localStorage.getItem('promoter_stats') || '{}');
